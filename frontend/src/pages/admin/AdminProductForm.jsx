      ...
      api.get(`/products/${id}`)
        .then(({ data }) => {
          setForm({
            ...
            category_ids: data.extra_categories?.length
              ? data.extra_categories.map(ec => ec.category?.id).filter(Boolean)
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-52 overflow-y-auto border border-stone-200 rounded-xl p-3 bg-stone-50">
              {categories.map(c => {
                ...
                return (
                  <label key={c.id}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-colors text-sm select-none ${checked ? 'bg-brand-100 border border-brand-400 text-brand-700 font-medium' : 'bg-white border border-stone-200 text-stone-600 hover:border-brand-300'}`}>
